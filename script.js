...
async function loadSchedule() {
  try {
    ...
    console.error(error);
    document.getElementById("weekGrid").innerHTML = "<p>Error al cargar el horario.</p>";
  }
}
...
function parseConfig(text) {
  ...
  text.split(/\r?\n/).forEach((rawLine) => {
    // Limpiar etiquetas de citación accidentales
    ...
    if (key === "nota") {
      ...
      if (type === "estudio") {
        label = key === "mañana" ? "Estudio de mañana" : "Estudio de tarde";
...
function renderSchedule(schedule) {
  ...
  orderedDays.forEach((day) => {
    ...
    if (data.note && data.note.trim() !== "") {
      ...
      noteDiv.innerHTML = `<span>📌</span> ${data.note}`;
...
function renderGeneralNotes(notes) {
  ...
  if (!list) return;
  list.innerHTML = notes.length === 0 ? "<li>No hay avisos.</li>" : notes.map(n => `<li>${n}</li>`).join("");
}
...
function cssClass(type) { return type === "study" || type === "estudio" ? "study" : (type === "rest" || type === "descanso" ? "rest" : "variable"); }
